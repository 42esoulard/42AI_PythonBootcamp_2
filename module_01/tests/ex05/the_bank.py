...
class Bank(object):
    ...
    def is_valid(self, account):
        ...
        if len(attributes) % 2 == 0 or \
           len(list(filter(lambda n: n.startswith('b'), attributes))) or \
           (not len(list(filter(lambda n: n.startswith('zip'), attributes))) and \
            not len(list(filter(lambda n: n.startswith('addr'), attributes)))) \
            or not 'name' in attributes or not 'id' in attributes or \
                not 'value' in attributes:
            return False
        return True
    ...
    def fix_account(self, account):
        ...
        if not len(list(filter(lambda n: n.startswith('zip'), attributes))) and \
            not len(list(filter(lambda n: n.startswith('addr'), attributes))):
